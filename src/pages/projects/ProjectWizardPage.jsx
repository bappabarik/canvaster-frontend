    ...
    <div className="flex items-center gap-0 mb-8 overflow-x-auto pb-2">
      {steps.map((s, i) => (
        <div key={i} className="flex items-center shrink-0">
          ...
          {i < steps.length - 1 && <div className="w-6 h-px bg-gray-300 shrink-0" />}
        </div>
      ))}
    </div>
  )
...
function StepTemplate({ onNext }) {
  ...
  function handleNext() {
    ...
    onNext({ templateId: selected.id, name: name.trim(), outputFormat: format })
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <input className="input max-w-md" placeholder="e.g. Class 10A ID Cards"
      ...
      <div>
        ...
        <div className="flex gap-3 flex-wrap">
          {[['zip_png','PNG (recommended)'],['zip_pdf','PDF'],['png','Single PNG'],['pdf','Single PDF']].map(([val, lbl]) => (
            <button key={val} onClick={() => setFormat(val)}
              className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${
                format === val ? 'border-brand-600 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
              }`}>
              {lbl}
            </button>
          ))}
      ...
      <div>
        <div className="flex gap-1 mb-4">
          {[['mine','My templates'],['gallery','Gallery']].map(([key, lbl]) => (
        ...
        {list.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            {tab === 'mine' ? 'No templates yet. Create one first.' : 'No gallery templates available.'}
          </div>
        ) : (
            ...
            {list.map(t => (
              <button key={t.id} onClick={() => setSelected(t)}
                className={`card overflow-hidden text-left transition-all hover:shadow-md ${
                  selected?.id === t.id ? 'ring-2 ring-brand-500 border-brand-200' : ''
                }`}>
                ...
                <div className="p-2">
                  ...
                  <p className="text-xs text-gray-400 capitalize">{t.category?.replace('_',' ')}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="flex justify-end">
        <button onClick={handleNext} className="btn-primary">Next: Upload CSV →</button>
      </div>
    </div>
  )
}
      ...
      {preview && (
        <div className="space-y-4">
          <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">
            ✓ {preview.total_rows} rows loaded · Headers: {preview.headers?.join(', ')}
          </div>
              ...
              <tbody>
                {preview.preview?.slice(0, 3).map((row, i) => (
                  <tr key={i} className="border-t border-gray-100">
                    {preview.headers?.map(h => (
                      <td key={h} className="px-3 py-2 text-gray-700 whitespace-nowrap max-w-[140px] truncate">{row[h]}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
          ...
          <div className="flex justify-between">
            ...
            <button onClick={() => onNext(preview)} className="btn-primary">Next: Map columns →</button>
          </div>
        </div>
      )}
...
function StepMap({ project, csvData, onNext, onBack }) {
  ...
  async function handleNext() {
    ...
    if (missing.length) { setError(`Map all placeholders: ${missing.join(', ')}`); return }
    setLoading(true)
    setError('')
    try {
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={autoMap} className="btn-secondary text-xs py-1.5">Auto-map</button>
      </div>
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div className="rounded-lg bg-blue-50 border border-blue-200 px-4 py-3 text-sm text-blue-700">
        <strong>Optional step.</strong> Only needed if your CSV has image filenames (e.g. <code>john.jpg</code>).
        If your CSV has direct image URLs, skip this step.
      </div>

      <div>
        <label className="label">Upload image ZIP</label>
        <p className="text-sm text-gray-500 mb-3">
          ZIP must contain image files named exactly as they appear in your CSV.
        </p>
        ...
        <div className="space-y-3">
          <div className={`rounded-lg border px-4 py-3 text-sm ${
            ...
            {report?.fully_matched
              ...
              : `${report?.unmatched_count} CSV rows have no matching image.`
            }
          </div>
            ...
            <div className="text-xs text-gray-500">
              ...
              <div className="flex flex-wrap gap-1">
                {report.unmatched_files.map(f => (
                  <span key={f} className="bg-yellow-50 border border-yellow-200 px-2 py-0.5 rounded font-mono">{f}</span>
                ))}
              </div>
            </div>
          )}
      ...
      <div className="flex justify-between">
        <button onClick={onBack} className="btn-secondary">← Back</button>
        <button onClick={onNext} className="btn-primary">Next: Review & submit →</button>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-6 max-w-xl">
      <div className="card p-5 space-y-3">
        ...
        {[
          ...
          ['Credits needed', project.total_rows],
        ].map(([label, val]) => (
      ...
      <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-700">
        Submitting will move this project to <strong>pending payment</strong>.
        You'll be redirected to complete payment before generation starts.
      </div>
...
export default function ProjectWizardPage() {
  ...
  async function handleTemplateNext({ templateId, name, outputFormat }) {
    ...
    try {
      ...
      setError(err.response?.data?.error?.description || 'Failed to create project')
    } finally {
      setCreating(false)
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {creating && <div className="flex justify-center py-8"><Spinner className="w-8 h-8" /></div>}
      ...
      {!creating && (
        <>
          ...
          {step === 1 && project && (
            <StepCsv
              project={project}
              onNext={data => { setCsvData(data); setStep(2) }}
              onBack={() => setStep(0)}
            />
          )}
          {step === 2 && project && (
            <StepMap
              project={project}
              csvData={csvData}
              onNext={map => { setColumnMap(map); setStep(3) }}
              onBack={() => setStep(1)}
            />
          )}
          {step === 3 && project && (
          ...
          {step === 4 && project && (
            <StepSubmit
              project={project}
              columnMap={columnMap}
              onSubmitted={() => navigate('/pricing', { state: { projectId: project.id, rows: project.total_rows } })}
              onBack={() => setStep(3)}
            />
          )}
        </>
