    ...
    <div className="card p-4 sm:p-5 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            ...
            {project.output_format?.replace("_", " ")}
          </p>
        </div>
      ...
      <div className="flex gap-2 mt-4">
        {project.status === "draft" && (
            <button
              onClick={() =>
                navigate("/projects/new", { state: { continueId: project.id } })
              }
        ...
        {["draft", "pending_payment", "queued", "processing", "done"].includes(
          project.status,
        ) &&
...
export default function ProjectsPage() {
  ...
  const filtered =
    filter === "all"
      ? projects
      : filter === "active"
        ? projects.filter((p) => ["queued", "processing"].includes(p.status))
